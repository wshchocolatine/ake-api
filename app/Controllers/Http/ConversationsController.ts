    ...
    public async New({ response, request, auth }: HttpContextContract): Promise<void> {
        ...
        */
        ...
        try {
            ...
            await trx.rollback()
            return response.internalServerError({ status: "Internal Server Error", errors: { message: 'Error at transaction' } })
        }
    ...
    */
    ...
    public async Get({ request, response, auth, session }: HttpContextContract): Promise<Array<object> | void> {
        ...
        */
        ...
        if(isNaN(offsetInt)) {
            return response.badRequest({ status: 'Bad Request', errors: { message: 'Offset parameter is missing or is invalid' } })
        }
    ...
    public async Search({ request, response, auth, session }: HttpContextContract): Promise<any> {
        ...
        */
        ...
        if (query === undefined || isNaN(offsetInt) ) {
            console.log('salut')
            return response.badRequest({ status: "Bad Request", errors: { message: 'Some parameters are invalid or missing' } })
        }
